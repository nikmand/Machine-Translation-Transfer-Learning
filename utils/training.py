...
def get_total_steps(dataset, cfg):
    """
    Args:
        dataset: (modeling.dl.dataloaders.text.TextDataset): dataset containing
        input text data, labels, transform, total classes, max length
        cfg: configuration file

    Returns:
        Number of total steps of optimization

    """
    batch = cfg["train_batch_size"]
    grad_acc = cfg["gradient_accumulation_steps"]
    epochs = cfg["epochs"]
    return int(len(dataset[0]) / batch / grad_acc) * epochs
